...
import { eq, like, and, count, desc, isNull } from 'drizzle-orm';
...
export const GET: APIRoute = async ({ cookies, request }) => {
  ...
  try {
    ...
    const conditions = and(
      isNull(moleculas.deletedAt),
      ...(filters.activo !== undefined && filters.activo !== ''
        ? [eq(moleculas.activo, filters.activo as '0' | '1')]
        : []),
      ...(search
        ? [like(moleculas.nombre, `%${search}%`)]
        : []),
    );
    ...
    console.error('Moleculas GET error:', err);
    return errorResponse('Error al obtener moleculas', 500);
  }
};
...
export const POST: APIRoute = async ({ cookies, request }) => {
  ...
  try {
    ...
    const [created] = await db.insert(moleculas).values({
    ...
    const [record] = await db
      .select()
      .from(moleculas)
      .where(eq(moleculas.id, Number(created.insertId)));

    return successResponse(record, 201);
  } catch (err: any) {
    console.error('Moleculas POST error:', err);
    if (err?.code === 'ER_DUP_ENTRY') {
      return errorResponse('Ya existe una molecula con ese nombre', 409);
    }
    return errorResponse('Error al crear molecula', 500);
  }
};
